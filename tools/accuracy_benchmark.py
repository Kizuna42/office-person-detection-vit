...
def compare_results(baseline: Dict, improved: Dict) -> Dict:
    ...
    comparison = {
        'success_rate': {
            ...
            'improved': improved.get('success_rate', 0.0) > baseline.get('success_rate', 0.0),
        },
        'avg_confidence': {
            ...
            'improved': improved.get('avg_confidence', 0.0) > baseline.get('avg_confidence', 0.0),
        },
        'valid_timestamps': {
            ...
            'improved': improved.get('valid_timestamps', 0) > baseline.get('valid_timestamps', 0),
        },
    }
    
    # 総合評価（すべての指標が改善または維持）
    overall_improved = (
        comparison['success_rate']['improved'] or 
        comparison['success_rate']['delta'] >= -1.0  # 1%以内の低下は許容
    ) and (
        comparison['avg_confidence']['improved'] or
        comparison['avg_confidence']['delta'] >= -0.01  # 0.01以内の低下は許容
    ) and (
        comparison['valid_timestamps']['improved'] or
        comparison['valid_timestamps']['delta'] >= -1  # 1件以内の低下は許容
    )
...
def main():
    ...
    if args.baseline and args.compare:
        ...
        logger.info(f"抽出成功率: {baseline.get('success_rate', 0.0):.2f}% → {improved.get('success_rate', 0.0):.2f}% (Δ{comparison['success_rate']['delta']:+.2f}%)")
        logger.info(f"平均信頼度: {baseline.get('avg_confidence', 0.0):.4f} → {improved.get('avg_confidence', 0.0):.4f} (Δ{comparison['avg_confidence']['delta']:+.4f})")
        logger.info(f"有効タイムスタンプ: {baseline.get('valid_timestamps', 0)} → {improved.get('valid_timestamps', 0)} (Δ{comparison['valid_timestamps']['delta']:+d})")
        logger.info("")
