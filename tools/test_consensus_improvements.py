...
class ImprovedConsensusOCR(MultiEngineOCR):
    ...
    def extract_with_weighted_consensus(
        self, roi: np.ndarray
...
def test_consensus_methods(
    ...
    # 各手法でテスト
    for method_name, ocr_engine in tqdm(methods.items(), desc="コンセンサス手法テスト中"):
        logger.info(f"コンセンサス手法 '{method_name}' をテスト中...")

        method_results = []
        parse_success_count = 0

        for frame_idx, frame in tqdm(frames, desc=f"  {method_name}処理中", leave=False):
            # ROI抽出
            ...
            if method_name == "weighted":
                ocr_text, ocr_confidence = ocr_engine.extract_with_weighted_consensus(
                    preprocessed
                )
            elif method_name == "voting":
                ocr_text, ocr_confidence = ocr_engine.extract_with_voting(preprocessed)
            else:
                ocr_text, ocr_confidence = ocr_engine.extract_with_consensus(
                    preprocessed
                )
            ...
            try:
                parsed_timestamp = (
                    timestamp_parser.parse(ocr_text) if ocr_text else None
                )
                if parsed_timestamp:
        ...
        # 統計を計算
        avg_confidence = (
            sum(r["ocr_confidence"] for r in method_results) / len(method_results)
            if method_results
            else 0.0
        )
        parse_success_rate = (
            (parse_success_count / len(method_results) * 100) if method_results else 0.0
        )
        ...
        logger.info(
            f"  {method_name}: 平均信頼度={avg_confidence:.4f}, パース成功率={parse_success_rate:.2f}%"
        )
