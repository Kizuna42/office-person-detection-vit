...
def verify_sample_video(video_path: str, config_path: str = "config.yaml"):
    ...
    try:
        ...
        logger.info(f"✓ メモリ使用量: {memory_before:.1f}MB → {memory_after:.1f}MB (増加: {memory_after - memory_before:.1f}MB)")
...
def main():
    ...
    parser.add_argument("--mode", choices=["sample", "production"], required=True,
    ...
    parser.add_argument("--duration", type=int, default=3600,
                       help="本番動画モードでの処理時間（秒、デフォルト: 3600）")
