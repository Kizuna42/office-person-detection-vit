...
def preprocess_with_fine_params(
    ...
    if sharpen_strength > 0:
        kernel_sharpen = np.array([[0, -sharpen_strength, 0],
...
def test_fine_tuning(
    ...
    for params in tqdm(param_sets, desc="パラメータセットテスト中"):
        ...
        logger.info(f"  平均信頼度: {avg_confidence:.4f}, 成功数: {success_count}/{len(param_results)}, 有効形式: {valid_count}/{len(param_results)}")
