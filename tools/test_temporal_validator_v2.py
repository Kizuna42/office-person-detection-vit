...
def test_validators(
    ...
    # 各バリデーターでテスト
    for validator_name, validator in tqdm(validators.items(), desc="バリデーターテスト中"):
        ...
        for frame_idx, frame in tqdm(
            frames, desc=f"  {validator_name}処理中", leave=False
        ):
            # ROI抽出
            ...
            # 時系列検証
            is_valid, temporal_confidence, reason = validator.validate(
                timestamp, frame_idx
            )
        ...
        valid_rate = (
            (valid_count / len(validator_results) * 100) if validator_results else 0.0
        )
        ...
        logger.info(
            f"  {validator_name}: 有効率={valid_rate:.2f}%, 平均信頼度={avg_confidence:.4f}"
        )
...
def main():
    ...
    if args.output:
        ...
        output_dir = (
            Path(config.get("output.directory", "output")) / "temporal_validator_test"
        )
    ...
    confidence_delta = improved.get("avg_confidence", 0.0) - baseline.get(
        "avg_confidence", 0.0
    )
