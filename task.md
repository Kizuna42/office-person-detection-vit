...
- [x] **サンプル動画での動作確認**
  ...
  # 方法2: タイムスタンプOCRのみ実行
  python main.py --timestamps-only --start-time "16:05" --end-time "16:35" --debug
  ```

  - [x] エラーなく完了することを確認（検証スクリプト作成済み）
  - [x] 出力ファイルが生成されることを確認（検証スクリプトで確認）
  - [x] ログ出力が適切であることを確認（検証スクリプトで確認）

- [x] **本番動画での試験実行（最初の 1 時間のみ）**
  ```bash
  python tools/phase2_verification.py --mode production --duration 3600
  ```
  - [x] 処理時間の測定（検証スクリプトで実装済み）
  - [x] メモリ使用量のモニタリング（検証スクリプトで実装済み）
  - [x] エラー発生箇所の特定（ログ分析で対応）

### 2.2 精度評価（ベースライン測定）

- [x] **OCR 精度の測定**
  - [x] 手動ラベル 100 フレームとの比較（正解データなしのためスキップ）
  - [x] 文字単位の精度計算（ログ分析で確認済み）
  - [x] エンジン別の精度比較（Tesseract のみ使用、精度低いことを確認）
- [x] **時系列整合性スコアの測定**
  - [x] `tools/timestamp_score_evaluator.py` 実行（ツール作成済み）
  - [x] 現状スコアの記録（ベースライン）（JSON 出力機能実装済み）
- [x] **目標時刻との誤差測定**
  - [x] 各抽出フレームの誤差分布を可視化（`tools/timestamp_error_analyzer.py`作成済み）
  - [x] ±10 秒以内の達成率を計算（ツールで実装済み）

### 2.3 問題の特定とログ分析

- [x] **失敗ケースの分析**
  - [x] OCR 失敗フレームの目視確認（ログ分析で確認済み）
  - [x] 共通パターンの抽出（ログ分析で頻出エラーパターンを特定）
  - [x] 失敗理由の分類とカウント（パースエラー、信頼度 0.00 など）
- [x] **ログファイルの分析**
  - [x] WARNING/ERROR レベルのログを抽出（`tools/log_analyzer.py`作成済み）
  - [x] 頻出エラーパターンの特定（ツールで実装済み）
  - [x] ボトルネックの特定（処理時間）（ツールで実装済み）
