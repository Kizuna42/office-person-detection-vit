...
def test_fuzzy_parse_missing_space(parser: TimestampParser):
    """スペース欠落の補正テスト"""
    # 例: "2025/08/2616:04:16" -> "2025/08/26 16:04:16"
    text = "2025/08/2616:04:16"
    dt, confidence = parser.fuzzy_parse(text)

    assert dt is not None
    assert dt == datetime(2025, 8, 26, 16, 4, 16)
    assert confidence == 1.0


def test_fuzzy_parse_missing_first_slash(parser: TimestampParser):
    """最初のスラッシュ欠落の補正テスト"""
    # 例: "2025108/26 16:04:16" -> "2025/08/26 16:04:16" または "2025/10/26 16:04:16"
    # 注意: このパターンは実際のログでは稀。主な問題はスペース欠落
    text = "2025108/26 16:04:16"
    dt, confidence = parser.fuzzy_parse(text)

    # パースが成功すればOK（年/月/日の解釈は複数パターンあり得る）
    assert dt is not None
    assert confidence == 1.0
    # 年と日は一致することを確認
    assert dt.year == 2025
...
def test_fuzzy_parse_missing_second_slash(parser: TimestampParser):
    """2番目のスラッシュ欠落の補正テスト"""
    # 例: "2025/0826 16:04:16" -> "2025/08/26 16:04:16"
    text = "2025/0826 16:04:16"
    dt, confidence = parser.fuzzy_parse(text)
...
def test_fuzzy_parse_missing_all_slashes(parser: TimestampParser):
    """全てのスラッシュ欠落の補正テスト"""
    # 例: "20250826 16:04:16" -> "2025/08/26 16:04:16"
    text = "20250826 16:04:16"
    dt, confidence = parser.fuzzy_parse(text)
...
def test_fuzzy_parse_combined_errors(parser: TimestampParser):
    """複数のエラーが組み合わさった場合のテスト"""
    # スペース欠落 + 文字誤認識
    text = "2025/08/26l6:04:16"
    dt, confidence = parser.fuzzy_parse(text)
