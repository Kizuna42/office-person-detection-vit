...
class TestCheckpointManager:
    ...
    def test_save_checkpoint_basic(self, manager, session_dir):
        """基本的なチェックポイント保存テスト"""
        manager.save_checkpoint("phase1_extraction", status="completed")
        ...
        assert "phase1_extraction" in data["phases"]
        assert data["phases"]["phase1_extraction"]["status"] == "completed"
        assert data["last_phase"] == "phase1_extraction"

    def test_save_checkpoint_with_data(self, manager):
        """データ付きチェックポイント保存テスト"""
        checkpoint_data = {"frames_processed": 100, "detections": 50}
        manager.save_checkpoint("phase2_detection", data=checkpoint_data, status="completed")

        with open(manager.checkpoint_path, encoding="utf-8") as f:
            data = json.load(f)

        assert data["phases"]["phase2_detection"]["data"]["frames_processed"] == 100
        assert data["phases"]["phase2_detection"]["data"]["detections"] == 50

    def test_save_checkpoint_multiple_phases(self, manager):
        """複数フェーズのチェックポイント保存テスト"""
        manager.save_checkpoint("phase1_extraction", status="completed")
        manager.save_checkpoint("phase2_detection", status="completed")
        manager.save_checkpoint("phase2.5_tracking", status="in_progress")
        ...
        assert data["phases"]["phase1_extraction"]["status"] == "completed"
        assert data["phases"]["phase2_detection"]["status"] == "completed"
        assert data["phases"]["phase2.5_tracking"]["status"] == "in_progress"

    def test_load_checkpoint_existing(self, manager):
        """既存チェックポイントの読み込みテスト"""
        checkpoint_data = {"frames_processed": 100}
        manager.save_checkpoint("phase1_extraction", data=checkpoint_data, status="completed")

        loaded = manager.load_checkpoint("phase1_extraction")
    ...
    def test_load_checkpoint_incomplete(self, manager):
        ...
        manager.save_checkpoint("phase1_extraction", status="in_progress")
    ...
    def test_is_phase_completed_true(self, manager):
        ...
        assert manager.is_phase_completed("phase1_extraction") is True

    def test_is_phase_completed_false(self, manager):
        """フェーズ完了確認テスト（未完了）"""
        manager.save_checkpoint("phase1_extraction", status="in_progress")

        assert manager.is_phase_completed("phase1_extraction") is False
    ...
    def test_get_last_completed_phase_single(self, manager):
        ...
        assert manager.get_last_completed_phase() == "phase1_extraction"

    def test_get_last_completed_phase_multiple(self, manager):
        """最後の完了フェーズ取得テスト（複数）"""
        manager.save_checkpoint("phase1_extraction", status="completed")
        manager.save_checkpoint("phase2_detection", status="completed")
        manager.save_checkpoint("phase2.5_tracking", status="completed")

        assert manager.get_last_completed_phase() == "phase2.5_tracking"
    ...
    def test_get_last_completed_phase_with_incomplete(self, manager):
        ...
        assert manager.get_last_completed_phase() == "phase2_detection"
    ...
    def test_get_resumable_phase_from_phase1(self, manager):
        """再開可能フェーズ取得テスト（phase1から）"""
        manager.save_checkpoint("phase1_extraction", status="completed")

        assert manager.get_resumable_phase() == "phase2_detection"

    def test_get_resumable_phase_from_phase2(self, manager):
        """再開可能フェーズ取得テスト（phase2から）"""
        manager.save_checkpoint("phase1_extraction", status="completed")
        manager.save_checkpoint("phase2_detection", status="completed")

        assert manager.get_resumable_phase() == "phase2.5_tracking"
    ...
    def test_get_summary_with_phases(self, manager, session_dir):
        ...
        assert summary["last_phase"] == "phase2_detection"
        assert "phase1_extraction" in summary["completed_phases"]
        assert "phase2_detection" in summary["completed_phases"]
        assert summary["resumable_from"] == "phase2.5_tracking"

    def test_checkpoint_persistence(self, session_dir):
        """チェックポイント永続化テスト"""
        # 最初のマネージャでチェックポイントを保存
        manager1 = CheckpointManager(session_dir)
        manager1.save_checkpoint("phase1_extraction", data={"test": "data"}, status="completed")

        # 新しいマネージャで読み込み
        manager2 = CheckpointManager(session_dir)
        loaded = manager2.load_checkpoint("phase1_extraction")
    ...
    def test_status_types(self, manager):
        """ステータスタイプのテスト"""
        manager.save_checkpoint("phase1_extraction", status="completed")
        manager.save_checkpoint("phase2_detection", status="in_progress")
        manager.save_checkpoint("phase2.5_tracking", status="failed")

        with open(manager.checkpoint_path, encoding="utf-8") as f:
            data = json.load(f)

        assert data["phases"]["phase1_extraction"]["status"] == "completed"
        assert data["phases"]["phase2_detection"]["status"] == "in_progress"
        assert data["phases"]["phase2.5_tracking"]["status"] == "failed"
    ...
    def test_timestamp_updated(self, manager):
        ...
        assert data["phases"]["phase1_extraction"]["timestamp"] is not None
